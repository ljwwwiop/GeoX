...
import geox.datasets.utils.cropping as cropping
from geox.utils.geometry import closed_form_inverse_se3, unproject_depth_map_to_point_map
from geox.utils.image import ImgNorm, imread_cv2
...
def apply_sky_segmentation(conf: np.ndarray, image_folder: str) -> np.ndarray:
    ...
    # Try to load skyseg.onnx from local pretrain_model directory first
    local_model_path = "./checkpoints/skyseg.onnx"

    # pdb.set_trace()
    if os.path.exists(local_model_path):
        print(f"Loading skyseg.onnx from local path: {local_model_path}")
        model_path = local_model_path
    elif os.path.exists("skyseg.onnx"):
        print("Using skyseg.onnx from current directory")
        model_path = "skyseg.onnx"
    else:
        print("Downloading skyseg.onnx from HuggingFace...")
        download_file_from_url(
...
def load_images_and_cameras(
    ...
    """
    # Get all image files (exclude sky_masks directories)
    image_paths = sorted(glob.glob(os.path.join(image_folder, "*")))
    image_paths = [
        p for p in image_paths
        if p.lower().endswith(('.png', '.jpg', '.jpeg'))
        and 'sky_mask' not in p.lower()  # Exclude sky mask files/folders
        and os.path.isfile(p)  # Only include files, not directories
    ]
    ...
    # Group by (width, height) to find the dominant orientation
    size_counts = {}
    for size in image_sizes:
        size_counts[size] = size_counts.get(size, 0) + 1

    # Find the most common size (dominant orientation)
    dominant_size = max(size_counts, key=size_counts.get)
    dominant_width, dominant_height = dominant_size

    print(f"\nImage size distribution: {size_counts}")
    print(f"Dominant size: {dominant_width}x{dominant_height}")

    # Detect images that need to be resized to match the dominant shape
    # For example: 387x512 needs to become 512x387 (not rotation, but resize)
    images_to_resize = []
    for idx, (img_path, size) in enumerate(zip(image_paths, image_sizes)):
        width, height = size
        if width != dominant_width or height != dominant_height:
            images_to_resize.append((idx, img_path, width, height))
            print(f"  [NOTE] Image {Path(img_path).stem} is {width}x{height} (needs resize to {dominant_width}x{dominant_height})")

    if images_to_resize:
        print(f"  Resizing {len(images_to_resize)} images to match dominant shape {dominant_width}x{dominant_height}")

    # Use the dominant size as the reference for all images
    # Ensure dimensions are multiples of 14 (ViT patch size requirement)
    ref_width = (dominant_width // 14) * 14
    ref_height = (dominant_height // 14) * 14
    ...
    # Create a set of indices that need resize for fast lookup
    resize_indices = set(idx for idx, _, _, _ in images_to_resize)
    ...
    for idx, img_path in enumerate(image_paths):
        ...
        if img.mode == "RGBA":
            ...
            img = Image.alpha_composite(background, img)
        img = img.convert("RGB")
        width, height = img.size  # PIL: width=W, height=H
        original_width, original_height = width, height

        # Check if image needs resize to match dominant shape
        # If dimensions are swapped (e.g., 387x512 instead of 512x387), we need to resize
        needs_resize = (width, height) != (dominant_width, dominant_height)

        if needs_resize:
            # Resize to the dominant dimensions first (maintaining aspect ratio)
            img = img.resize((dominant_width, dominant_height), Image.Resampling.BICUBIC)
            width, height = dominant_width, dominant_height
            print(f"  [RESIZED] {basename}: {original_width}x{original_height} -> {width}x{height}")

        # Now resize to the target size (ensure divisible by 14)
        new_width = target_size
        # Maintain aspect ratio to determine height
        scale = target_size / width
        new_height = round(height * scale / 14) * 14
