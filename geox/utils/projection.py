...
def get_world_rays(
    ...
    directions = directions / directions[..., -1:] # Project points to z=1 plane
    ...
    origins = extrinsics[..., :-1, -1].broadcast_to(directions.shape) # Extract translation vector T
...
def sample_voxel_grid(
    ...
    """
    Sample voxel grid center coordinates and their indices.
    
    Args:
        xbound: [xmin, xmax, step]
        ybound: [ymin, ymax, step]
        zbound: [zmin, zmax, step]
        device: torch device to use
    
    Returns:
        centers: (Z, Y, X, 3) float tensor of voxel center coordinates
        indices: (Z, Y, X, 3) int tensor of voxel grid indices
    """
    # Unpack bounds
    ...
    zbins = int((zmax - zmin) / zstep)

    # Indices (integer grid)
    x_idx = torch.arange(xbins, device=device)
    y_idx = torch.arange(ybins, device=device)
    z_idx = torch.arange(zbins, device=device)
    grid_idx = torch.stack(torch.meshgrid(z_idx, y_idx, x_idx, indexing="ij"), dim=-1)  # shape [Z, Y, X, 3]

    # Convert to float coordinates (center positions)
    x_centers = xmin + (x_idx + 0.5) * xstep
    y_centers = ymin + (y_idx + 0.5) * ystep
    z_centers = zmin + (z_idx + 0.5) * zstep
    grid_centers = torch.stack(torch.meshgrid(x_centers, y_centers, z_centers, indexing="ij"), dim=-1)  # shape [Z, Y, X, 3]
    return grid_centers, grid_idx
