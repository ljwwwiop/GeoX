...
from geox.utils.general import check_and_fix_inf_nan
...
def normalize_camera_extrinsics_scale_and_points_batch(
    extrinsics: torch.Tensor,
    cam_points: Optional[torch.Tensor] = None,
    world_points: Optional[torch.Tensor] = None,
    depths: Optional[torch.Tensor] = None,
    ...
    """
    Normalize camera extrinsics and corresponding 3D points, AND return the scale factor.
    Same as normalize_camera_extrinsics_and_points_batch but also returns the scale.
    ...
    Returns:
        ...
        - Normalized world points (same shape as input world_points)
        - Normalized depths (same shape as input depths)
        - Scene scale factors of shape (B,) - multiply normalized depth by this to get metric depth
    """
    # Validate inputs
    check_valid_tensor(extrinsics, "extrinsics")
    check_valid_tensor(cam_points, "cam_points")
    ...
    avg_scale = torch.ones(B, device=device)  # default scale = 1
    ...
    if scale_by_points:
        ...
        if new_world_points is not None and point_masks is not None:
            dist = new_world_points.norm(dim=-1)
            dist_sum = (dist * point_masks).sum(dim=[1,2,3])
            valid_count = point_masks.sum(dim=[1,2,3])
            avg_scale = (dist_sum / (valid_count + 1e-3)).clamp(min=1e-6, max=1e6)
