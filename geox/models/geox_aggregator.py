...
from geox.utils.pose_enc import extri_intri_to_pose_encoding
from torch.utils.checkpoint import checkpoint
from geox.utils.geometry import closed_form_inverse_se3
from geox.models.aggregator import Aggregator, slice_expand_and_flatten
...
class GeoXAggregator(Aggregator):
    def __init__(self, img_size=518,
        ...
        # # Override the base class camera_token and register_token initialization
    ...
    def inference(self, images: torch.Tensor,
        ...
        if len(camera_gt_index) != 0:
            ...
            camera_idx_tensor = torch.tensor(camera_gt_index, device=images.device)
            ...
            pose_encoding = None
            gt_camera_token = torch.zeros(K, 1, C, device=images.device, dtype=camera_token.dtype)
            ...
            for attn_type in self.aa_order:
                if attn_type == "frame":
                    tokens, frame_idx, frame_intermediates = self._process_frame_attention(
                        ...
                        pose_encoding=pose_encoding, register_shape=register_token.shape
                    )
    ...
    def _process_frame_attention(self, tokens, B, S, P, C, frame_idx, pos=None, index=None, camera_gt_index=None,
                         pose_encoding=None, register_shape=None):
        """
        ...
        injection_tokens = torch.cat([camera_injection, register_token], dim=1)
        ...
        for _ in range(self.aa_block_size):
            ...
            tokens = torch.cat([
                tokens[:, :self.patch_start_idx] + injection_tokens,
